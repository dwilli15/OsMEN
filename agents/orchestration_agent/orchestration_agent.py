...
class OrchestrationAgent:
    ...
    def _handle_secret_request(self, team: str, msg: Dict[str, Any]) -> None:
        ...
        Uses environment variables, .env file, or Windows Credential Manager
        to retrieve requested secrets. Falls back to prompting user if not found.
        """
        import keyring
        ...
        # 2. Try .env file if not in environment
        if not secret_value:
        ...
        # 3. Try Windows Credential Manager / Keyring
        if not secret_value:
            try:
                secret_value = keyring.get_password("OsMEN", secret_name)
                if secret_value:
                    source = "keyring"
            except Exception as e:
                logger.debug(f"Keyring lookup failed: {e}")

        # 4. Try common variations of the secret name
        if not secret_value:
            variations = [
                secret_name,
                secret_name.lower(),
                secret_name.upper(),
                f"{secret_name}_KEY",
                f"{secret_name}_SECRET",
                f"{secret_name}_TOKEN",
                f"{secret_name}_API_KEY",
            ]
            for var in variations:
                var_env = var.upper().replace(" ", "_").replace("-", "_")
                secret_value = os.getenv(var_env)
                if secret_value:
                    source = f"environment ({var_env})"
                    break

        if secret_value:
            # Secret found - send it securely to the requesting team
            logger.info(f"Secret '{secret_name}' found via {source}")
            self.send_message(
                team,
                {
            ...
            # Secret not found - notify user needs to provide it
            logger.warning(f"Secret '{secret_name}' not found in any source")

            # Store pending request for later retrieval
            pending_file = self.message_dir / "pending_secrets.json"
            pending = []
            ...
            self.send_message(
                team,
                {
                    ...
                    "payload": {
                        "secret_name": secret_name,
                        "env_name": env_name,
                        "message": f"Secret not found. User needs to set {env_name} environment variable or add to .env file.",
                        "instructions": [
                            f"Option 1: Set environment variable: export {env_name}=<your_value>",
                            f"Option 2: Add to .env file: {env_name}=<your_value>",
                            f"Option 3: Use keyring: keyring set OsMEN {secret_name}",
                        ],
    ...
    def get_sprint_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_tasks_completed": sum(
                len(t.get("tasks_completed", [])) for t in self.teams.values()
            ),
        }
    ...
    def run_coordination_loop(self, iterations: int = 10, interval: int = 30) -> None:
        ...
        """
        logger.info(
            f"Starting orchestration loop: {iterations} iterations, {interval}s interval"
        )
        self.status = "running"
        ...
        for i in range(iterations):
            ...
            logger.info(
                f"Sprint status: {status['total_tasks_completed']} tasks completed, "
