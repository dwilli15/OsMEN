...
class BootHardeningAgent:
    ...
    # Registry locations for startup programs (populated in __init__ if winreg is available)
    ...
    def __init__(self):
        ...
        # Initialize registry paths if on Windows
        if WINREG_AVAILABLE and winreg:
    ...
    def _scan_registry_startup(self) -> List[StartupItem]:
        ...
        if not WINREG_AVAILABLE or not winreg:
            return items
    ...
    def _check_suspicious(self, item: StartupItem):
        ...
        Modifies the item in place by setting suspicious and reason fields.
        
        Args:
            item: StartupItem to check and annotate.
        """
    ...
    def _add_windows_firewall_rule(self, rule: Dict) -> bool:
        ...
        Args:
            rule: Dictionary containing rule configuration with keys:
                - name: Rule name (required)
                - action: 'block' or 'allow' (default: block)
                - direction: 'in', 'out', or 'both' (default: both)
                - program: Program path (optional)
        ...
        Returns:
            True if rule was successfully added, False otherwise.
        """
        name = rule.get('name')
        action = rule.get('action', 'block')
        direction = rule.get('direction', 'both')
        ...
        if not re.match(r'^[\w\-]+$', name):
            logger.error(f"Invalid characters in rule name: {name}")
            return False
        ...
        for dir_val in directions:
            netsh_cmd = [
                ...
                f'name=OsMEN_{name}_{dir_val}',
                f'dir={dir_val}',
                f'action={action}'
            ]
            
            if rule.get('program'):
                program_path = rule['program']
                # Validate program path - must be an absolute path with valid characters
                if not isinstance(program_path, str) or not Path(program_path).is_absolute():
                    logger.error(f"Invalid program path: {program_path}")
                    return False
                # Check for shell metacharacters
                if any(c in program_path for c in ['|', '&', ';', '$', '`', '>', '<']):
                    logger.error(f"Invalid characters in program path: {program_path}")
                    return False
                netsh_cmd.append(f"program={program_path}")
            ...
            if rule.get('port'):
                ...
                try:
                    ...
                    return False
                netsh_cmd.append(f"localport={port_int}")
                protocol = rule.get('protocol', 'TCP')
                if protocol not in ('TCP', 'UDP'):
                    logger.error(f"Invalid protocol: {protocol}")
                    return False
                netsh_cmd.append(f"protocol={protocol}")
    ...
    def get_security_events(self, hours: int = 24, max_events: int = 100) -> List[Dict]:
        ...
        Args:
            hours: Look back this many hours (must be positive integer)
            max_events: Maximum events to return (must be positive integer)
        
        Returns:
            List of security event dictionaries
        
        Raises:
            ValueError: If hours or max_events are not positive integers
        """
