...
class AggregatedContext:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "today_am_checkin": vars(self.today_am_checkin) if self.today_am_checkin else {},
            "yesterday_pm_checkin": vars(self.yesterday_pm_checkin) if self.yesterday_pm_checkin else {},
...
class CheckInParser:
    ...
    def parse_content(content: str, filename: str) -> CheckInData:
        """Parse check-in markdown content"""
        # Extract date and period from filename (e.g., "2025-12-11-AM")
        ...
        # Extract energy level (check for checked box pattern [x])
        energy_patterns = [
            ...
            (r"Energy Level[:\s]*(\d+)/10", None),  # Numeric format
            (r"Energy[:\s]*(\d+)", None),
        ]
        
        for pattern, fixed_value in energy_patterns:
        ...
        # Extract priorities - look for numbered items with actual content
        priority_section = re.search(
            r"(?:Top 3 Priorities|Priorities Today).*?\n((?:\d+\..+?\n)+)",
            content, re.IGNORECASE | re.DOTALL
        )
        if priority_section:
            raw_priorities = re.findall(r"\d+\.\s*(.+?)(?:\n|$)", priority_section.group(1))
            # Filter out empty priorities and markdown artifacts
            data.priorities = [
                p.strip() for p in raw_priorities 
                if p.strip() and not p.strip().startswith('#') and len(p.strip()) > 1
            ]
        
        # Extract focus blocks - look for actual task descriptions
        ...
        if focus_section:
            ...
            # Match format: - [ ] Deep Work Block N (90 min): <task>
            focus_blocks = re.findall(
                r"\[([x ])\]\s*Deep Work Block \d+.*?:\s*(.+?)(?:\n|$)",
                focus_lines, re.IGNORECASE
            )
            # Only include blocks that have actual task descriptions
            data.focus_blocks_planned = [
                fb[1].strip() for fb in focus_blocks 
                if fb[1].strip() and not fb[1].strip().startswith('-') and len(fb[1].strip()) > 1
            ]
        ...
        for pattern, state in adhd_states:
        ...
        if period == "PM":
            # Productivity rate
            productivity = re.search(r"(?:Productivity|Overall productivity)[:\s]*(\d+)%?", content, re.IGNORECASE)
            if productivity:
                data.productivity_rate = int(productivity.group(1))
            
            # Accomplishments (list items with actual content)
            accomplishments_section = re.search(
                r"(?:### Accomplishments|## Accomplishments).*?\n((?:\d+\..+?\n)+)",
                content, re.IGNORECASE | re.DOTALL
            )
            if accomplishments_section:
                raw_acc = re.findall(r"\d+\.\s*(.+?)(?:\n|$)", accomplishments_section.group(1))
                data.accomplishments = [a.strip() for a in raw_acc if a.strip() and len(a.strip()) > 1]
            
            # Carryover tasks (incomplete items)
            carryover_section = re.search(
                r"(?:### Incomplete Items|Carryover).*?\n((?:[-*]\s*.+?\n)+)",
                content, re.IGNORECASE | re.DOTALL
            )
            if carryover_section:
                raw_carryover = re.findall(r"[-*]\s*(.+?)(?:\n|$)", carryover_section.group(1))
                data.carryover_tasks = [c.strip() for c in raw_carryover if c.strip() and len(c.strip()) > 1]
            ...
            if tomorrow_section:
                raw_tomorrow = re.findall(r"\d+\.\s*(.+?)(?:\n|$)", tomorrow_section.group(1))
                tomorrow_tasks = [t.strip() for t in raw_tomorrow if t.strip() and len(t.strip()) > 1]
                if tomorrow_tasks:
...
class ContextAggregator:
    ...
    def gather_full_context(self) -> AggregatedContext:
        ...
        for checkin in context.checkins:
            ...
            elif checkin.date == (self.today - timedelta(days=1)).isoformat() and checkin.period == "PM":
        ...
        context.carryover_from_yesterday = self._get_carryover(context.yesterday_pm_checkin)
    ...
    def _gather_briefings(self) -> List[BriefingData]:
        ...
        for days_ago in range(1, self.lookback_days + 1):
            ...
            if script_path.exists():
                ...
                briefings.append(BriefingData(
                    date=script_date.isoformat(),
                    word_count=len(content.split()),
                    content=content,
                    # Extract priorities mentioned
                    key_priorities=re.findall(r"number one today[:\s]*([^.]+)", content, re.IGNORECASE),
                ))
    ...
    def _calculate_adhd_context(self, checkins: List[CheckInData]) -> ADHDContext:
        ...
        # Calculate 3-day average energy
        ...
        for checkin in checkins:
            if checkin.notes:
                # Look for positive patterns mentioned
                if "body doubling" in checkin.notes.lower():
                    context.working_strategies.append("body doubling")
                if "timer" in checkin.notes.lower():
                    context.working_strategies.append("visual timer")
                if "walk" in checkin.notes.lower():
                    context.working_strategies.append("movement breaks")
    ...
    def _get_progress_context(self) -> ProgressContext:
        ...
        if progress_file.exists():
            ...
            context.readings_total = total_markers if total_markers > 0 else 50  # Default estimate
            ...
            if behind_match:
                ...
                if syllabus and syllabus.current_week > 0:
                    ...
                    context.readings_behind = max(0, expected - context.readings_completed)
        ...
        for checkin in self.aggregator_checkins if hasattr(self, 'aggregator_checkins') else []:
    ...
    def _aggregate_pending_tasks(self, context: AggregatedContext) -> List[str]:
        ...
        for checkin in context.checkins:
            if checkin.period == "AM":
                uncompleted = set(checkin.focus_blocks_planned) - set(checkin.focus_blocks_completed)
        ...
        if context.syllabus and context.syllabus.next_deadline:
            if context.syllabus.next_deadline["days_until"] <= 7:
                pending.append(f"Course: {context.syllabus.next_deadline['name']} (due in {context.syllabus.next_deadline['days_until']} days)")
...
if __name__ == "__main__":
    ...
    for ci in context.checkins:
        print(f"  - {ci.date} {ci.period}: Energy={ci.energy}, Priorities={len(ci.priorities)}")
    ...
    if context.syllabus:
        ...
        if context.syllabus.next_deadline:
            print(f"  Next deadline: {context.syllabus.next_deadline['name']} in {context.syllabus.next_deadline['days_until']} days")
    
    if context.adhd:
        print(f"\n🧠 ADHD Context:")
        print(f"  3-day avg energy: {context.adhd.avg_energy_3day:.1f}")
        print(f"  Trend: {context.adhd.energy_trend}")
        print(f"  Recommended focus blocks: {context.adhd.recommended_focus_block_length} min")
