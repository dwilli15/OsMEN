...
class FocusGuardrailsAgent:
    ...
    PRODUCTIVE_APPS = {
        'code.exe', 'devenv.exe', 'idea64.exe', 'pycharm64.exe',  # IDEs
        'notepad.exe', 'notepad++.exe', 'sublime_text.exe',  # Editors
        'excel.exe', 'word.exe', 'powerpnt.exe',  # Office
        'outlook.exe', 'teams.exe', 'slack.exe',  # Communication (work)
        'terminal.exe', 'powershell.exe', 'cmd.exe', 'WindowsTerminal.exe',  # Terminal
        'explorer.exe', 'taskmgr.exe',  # System
    }
        ...
        try:
            if self.state_file.exists():
                with open(self.state_file, 'r') as f:
                    ...
                    logger.info(f"Loaded state: {len(self.blocked_sites)} blocked sites")
        except Exception as e:
    ...
    def _save_state(self):
        ...
        try:
            state = {
                ...
                'sessions': [asdict(s) for s in self.focus_sessions[-20:]],  # Keep last 20
    ...
    def start_focus_session(self, duration_minutes: int = 25, block_sites: bool = True) -> Dict:
        """
        ...
        try:
            ...
            session = FocusSession(
                ...
                status='active',
                blocked_sites=list(self.DEFAULT_DISTRACTING_SITES) if block_sites else []
            )
            ...
            if block_sites:
                ...
                if not block_result['success']:
                    logger.warning(f"Some sites could not be blocked: {block_result['errors']}")
    ...
    def _add_to_hosts_file(self, sites: List[str]) -> Dict:
        ...
        try:
            ...
            result['errors'].append('Permission denied writing hosts file. Run as Administrator.')
        except Exception as e:
        ...
        if self.is_windows:
            for site in sites:
                try:
                    ...
                    result['errors'].append(f'Error removing firewall rule for {site}: {e}')
    ...
    def unblock_sites(self, sites: Optional[List[str]] = None) -> Dict:
        ...
        try:
            ...
            if not sites:
                return {'unblocked': [], 'status': 'success', 'message': 'No sites to unblock'}
            
            result = self._remove_site_blocks(sites)
            result['status'] = 'success' if not result.get('errors') else 'partial_success'
    ...
    def monitor_app_usage(self, duration_seconds: int = 60) -> Dict:
        ...
        try:
            ...
            if last_app:
                app_times[last_app] = app_times.get(last_app, 0) + (time.time() - last_switch)
