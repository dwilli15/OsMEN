...
class ResearchIntelAgent:
    ...
    def __init__(self, llm_provider: str = "ollama"):
        ...
        logger.info(f"ResearchIntelAgent initialized (RAG={self.use_rag}, LLM={self.use_llm})")
        ...
        if self.use_rag and self.librarian:
            try:
                ...
                if mode == 'lateral':
                    ...
                    research['lateral_connections'] = connections.get('domains_touched', [])
...
"""
                ...
                response = await llm.chat([
                    {"role": "system", "content": "You are a research analyst providing thorough, evidence-based summaries."},
        ...
        if not research['summary']:
            research['summary'] = f"Research summary for {topic} (generated without LLM)"
    ...
    def analyze_document(self, document_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if paragraphs:
                ...
                analysis['summary'] = first_para[:300] + '...' if len(first_para) > 300 else first_para
            
            # Ingest to librarian if available
            if self.use_rag and self.librarian:
                try:
                    ingest_result = self.librarian.ingest_documents(str(path), recursive=False)
    ...
    async def gather_intelligence_async(self, queries: List[str]) -> Dict[str, Any]:
        ...
        if llm and intelligence['findings']:
            try:
                ...
                response = await llm.chat([
                    {"role": "system", "content": "You are an intelligence analyst synthesizing multiple research findings."},
                    {"role": "user", "content": f"Synthesize the following research findings into a coherent intelligence report:\n\n{findings_text}"}
                ])
