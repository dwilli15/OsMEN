...
try:
    from agents.personal_assistant.assistant_memory import AssistantMemoryIntegration
except ImportError:
    AssistantMemoryIntegration = None  # type: ignore

# Configure logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Try to import assistant memory for personalization
ASSISTANT_MEMORY_AVAILABLE = False
try:
    from .assistant_memory import AssistantMemoryIntegration, get_assistant_memory

    ASSISTANT_MEMORY_AVAILABLE = True
    logger.info("Assistant memory integration available")
except ImportError as e:
    logger.warning(f"Assistant memory not available: {e}")
...
class PersonalAssistantAgent:
    ...
    def __init__(self, data_dir: str = "data/personal_assistant"):
        ...
        # Initialize assistant memory for personalization
        self.assistant_memory: Optional[AssistantMemoryIntegration] = None
        if ASSISTANT_MEMORY_AVAILABLE:
            try:
                self.assistant_memory = get_assistant_memory()
                logger.info("Assistant memory enabled for personalization")
            except Exception as e:
                logger.warning(f"Failed to initialize assistant memory: {e}")
        ...
        logger.info(
            f"PersonalAssistantAgent initialized (LLM={self.use_llm}, Calendar={self.use_calendar}, Memory={bool(self.assistant_memory)}, DB={self.db_path})"
        )
    ...
    def create_task(
        ...
        # Store in assistant memory for learning
        if self.assistant_memory and self.assistant_memory.enabled:
            try:
                self.assistant_memory.store_interaction(
                    interaction_type="task_creation",
                    content=f"Created task '{title}' with {priority} priority",
                    metadata={
                        "priority": priority,
    ...
    def set_reminder(self, title: str, time: str, message: str = "") -> Dict[str, Any]:
        """
    ...
    def get_daily_summary(self, date: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Add personalized recommendations if memory is available
        if self.assistant_memory and self.assistant_memory.enabled:
            try:
                summary["personalized_recommendations"] = (
                    self.assistant_memory.get_personalized_recommendations(
                        f"daily planning for {date}", n_recommendations=3
                    )
                )
            except Exception as e:
...
"""
                ...
                response = await llm.chat(
                    [
                        {
                            "role": "system",
                            "content": "You are a personal assistant that parses natural language into structured actions. Return only valid JSON.",
                        },
