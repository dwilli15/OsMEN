...
class TestMicrosoftOAuthHandler:
    ...
    def test_tenant_endpoint_configuration(self):
        """Test that endpoints are configured for specific tenant"""
        ...
        assert "12345678-1234-1234-1234-123456789012" in handler.auth_endpoint
        assert "12345678-1234-1234-1234-123456789012" in handler.token_endpoint
    
    def test_authorization_url_with_optional_params(self, handler):
        ...
        assert "domain_hint=example.com" in auth_url
        assert "login_hint=john" in auth_url or "john%40example.com" in auth_url
    
    def test_admin_consent_url_generation(self, handler):
        """Test admin consent URL generation"""
        consent_url = handler.get_admin_consent_url(state="test_state")
        
        assert handler.admin_consent_endpoint in consent_url
        assert "client_id=test_client_id" in consent_url
        assert "state=test_state" in consent_url
    
    @patch('requests.post')
    def test_exchange_code_for_token_failure(self, mock_post, handler):
        ...
        success = handler.exchange_code_for_token("invalid_code")
        
        assert success is False
        assert handler.access_token is None
    
    @patch('requests.post')
    def test_refresh_token_failure(self, mock_post, handler):
        """Test failed token refresh"""
        handler.refresh_token = "invalid_refresh_token"
    ...
    def test_refresh_token_no_refresh_token(self, handler):
    ...
    @patch('requests.get')
    def test_validate_token_success(self, mock_get, handler):
        """Test successful token validation"""
        handler.access_token = "valid_token"
        handler.token_expiry = datetime.now() + timedelta(hours=1)
        
        mock_response = Mock()
        mock_response.status_code = 200
        mock_get.return_value = mock_response
        
        is_valid = handler.validate_token()
        
        assert is_valid is True
    
    @patch('requests.get')
    def test_get_user_info_success(self, mock_get, handler, mock_user_info):
        ...
        assert user_info['mail'] == "john.doe@example.com"
    
    def test_parse_id_token(self, handler):
        """Test ID token parsing"""
        # Valid JWT (header.payload.signature)
        id_token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiZW1haWwiOiJqb2huQGV4YW1wbGUuY29tIn0.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c"
        
        user_info = handler._parse_id_token(id_token)
        
        assert user_info is not None
        assert user_info['name'] == "John Doe"
        assert user_info['email'] == "john@example.com"
    
    def test_is_authenticated_no_token(self, handler):
        """Test authentication check with no token"""
        handler.access_token = None
        
        assert handler.is_authenticated() is False
    
    def test_is_authenticated_valid_token(self, handler):
    ...
    def test_revoke_token(self, handler, tmp_path):
        """Test token revocation"""
        handler.access_token = "test_token"
        handler.refresh_token = "test_refresh"
        handler.token_expiry = datetime.now() + timedelta(hours=1)
        handler._save_token()
        
        assert handler.token_storage_path.exists()
        
        success = handler.revoke_token()
        
        assert success is True
        assert handler.access_token is None
        assert handler.refresh_token is None
        assert not handler.token_storage_path.exists()
    
    def test_token_persistence(self, handler, tmp_path):
        """Test token save and load"""
        handler.access_token = "test_access_token"
        handler.refresh_token = "test_refresh_token"
        handler.token_expiry = datetime.now() + timedelta(hours=1)
        handler.user_info = {"name": "Test User"}
        
        handler._save_token()
        
        new_handler = MicrosoftOAuthHandler(
            client_id="test_client_id",
            client_secret="test_client_secret",
            token_storage_path=str(handler.token_storage_path)
        )
        
        assert new_handler.access_token == "test_access_token"
        assert new_handler.refresh_token == "test_refresh_token"
        assert new_handler.user_info == {"name": "Test User"}
